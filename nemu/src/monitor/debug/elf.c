...
uint32_t find_addr(char* val,bool *is_obj) {
	int i=0;
	for(;i<nr_symtab_entry;i++)
	{
		if((strcmp(val,strtab+((symtab+i)->st_name))==0) && (((symtab+i)->st_info&0xf)==STT_OBJECT))
		{
...
void in_which_func(uint32_t addr)
{
	...
	for(;i<nr_symtab_entry;i++)
	{
		...
		{
			addr1=(symtab+i)->st_value;
			addr2=addr1+(symtab+i)->st_size;
