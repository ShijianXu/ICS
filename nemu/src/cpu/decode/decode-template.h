...
#define decode_rm1_internal concat3(decode_rm1_, SUFFIX, _internal)
...
/******************************************/
static int concat3(decode_rm1_, SUFFIX, _internal) (swaddr_t eip, Operand *rm, Operand *reg) {
	rm->size = 1;
	int len = read_ModR_M(eip, rm, reg);
	reg->val = REG(reg->reg);
...
/*******************************/
make_helper(concat(decode_rm12r_, SUFFIX)) {
	return decode_rm1_internal(eip, op_src, op_dest);
}
